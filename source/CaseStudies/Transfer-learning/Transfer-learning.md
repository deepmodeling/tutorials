...
This is where the "simplify" module in the [DP-GEN](https://github.com/deepmodeling/dpgen) software (version > 0.8.0) comes in. simplify is an automated active-learning workflow designed specifically to hunt down the most valuable structures for transfer learning. It works by iterating through these steps:
...
- Training (00.train): DP models are fine-tuned on the relabeled data using the init_model feature, freezing parameters like the embedding net while keeping the output layers of the fitting net trainable.
...
Regardless of how you obtained the new data (Approach A or B), the actual "learning" happens in [DeePMD-kit](https://github.com/deepmodeling/deepmd-kit). You utilize the init_model tag in your training parameter file to load your pre-trained model, and you freeze the necessary layers (usually the embedding net) so that the model only updates its fitting layers based on your new dataset.
...
If we want to test the simplify workflow, or if we pretend for a moment that D3 calculations are highly expensive, we can use DP-GEN to extract a minimal dataset.
...
The parameter file simplify.json is formatted similarly to a typical DP-GEN process. By running this, the simplify active learning loop actively hunts for the structures that represent the biggest difference between PBE and PBE-D3.
